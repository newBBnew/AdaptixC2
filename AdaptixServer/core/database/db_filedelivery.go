...
func (dbms *DBMS) DbFileDeliveryFileUpsert(fileID string, fileName string, sha256 string, size int64, owner string, createdAt int64, storedPath string) error {
	...
	query := `INSERT OR REPLACE INTO FileDeliveryFiles (FileId, FileName, Sha256, Size, Owner, CreatedAt, StoredPath) VALUES(?,?,?,?,?,?,?);`
...
func (dbms *DBMS) DbFileDeliveryFileGet(fileID string) (FileDeliveryFileRow, error) {
	...
	q := `SELECT FileId, FileName, Sha256, Size, Owner, CreatedAt, StoredPath FROM FileDeliveryFiles WHERE FileId = ?;`
...
func (dbms *DBMS) DbFileDeliveryFileDelete(fileID string) error {
	...
	_, err := dbms.database.Exec(`DELETE FROM FileDeliveryFiles WHERE FileId = ?;`, fileID)
	return err
}
...
func (dbms *DBMS) DbFileDeliveryFilesByOwner(owner string) ([]FileDeliveryFileRow, error) {
	...
	rows, err := dbms.database.Query(`SELECT FileId, FileName, Sha256, Size, Owner, CreatedAt, StoredPath FROM FileDeliveryFiles WHERE Owner = ? ORDER BY CreatedAt DESC;`, owner)
	if err != nil {
		return nil, err
	}
