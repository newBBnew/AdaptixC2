...
// inflightDelivery tracks a task batch that has been sent but not yet ACKed by the agent.
// Used by DNS/DoH listeners for reliable delivery over unreliable transport.
...
// tsGetOrCreateInflight returns an existing inflight delivery (for retransmission) or creates a new one.
func (ts *Teamserver) tsGetOrCreateInflight(agent *Agent, maxDataSize int) ([]byte, uint32, error) {
	...
	if oldest != nil {
		...
		respData, err := ts.Extender.ExAgentPackData(agentData, oldest.tasks)
	...
	if tasksCount > 0 {
		...
		ts.TsAgentConsoleOutput(agentData.Id, CONSOLE_OUT_INFO, message, "", false)
	}
	...
	for range ticker.C {
		...
		ts.Agents.ForEach(func(_ string, value interface{}) bool {
			...
			for _, k := range toRequeue {
				...
				// Requeue tasks back to HostedTasks (prepend to preserve order)
